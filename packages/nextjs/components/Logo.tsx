...
export const Logo: React.FC<Props> = ({ size = 48, className = "", showArcana = true }) => {
  ...
  const arcanaLogo = resolvedTheme === "dark" ? "/logos/arcana-white.svg" : "/logos/arcana-black.svg";
    ...
    <div className={`flex items-center ${className}`}>
      ...
      {showArcana && (
        <div className="flex items-center">
          <Image src={arcanaLogo} alt="Arcana" width={size * 2} height={size * 0.3} className="object-contain" />
        </div>
      )}
    </div>
  );
